...
const TripGenerator: React.FC<TripGeneratorProps> = ({ onBackToExplore, onShowPlans }) => {
  ...
  const handleOriginConfirm = (originData: { country: string; city: string }) => {
    setOrigin({
  ...
  <div className="planner-layout">
    {/* Modal de origen obligatorio */}
    <OriginModal
      isOpen={showOriginModal}
      onConfirm={handleOriginConfirm}
    />

    {/* Solo mostrar contenido si ya se seleccionó origen */}
    {origin ? (
      <>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="form-container">
            ...
            <button
             ...
             className={`generate-button w-full ${!isFormValid || isGenerating ? '' : ''}
                isFormValid && !isGenerating
                  ? 'btn-primary hover:shadow-xl hover:scale-105 text-white'
      ...
      /* Pantalla de bienvenida mientras se selecciona origen */
      <div className="planner-header" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="planner-title">Bienvenido a TripWase</h1>
          <p className="planner-subtitle mb-8">
            Para comenzar a planificar tu viaje perfecto, necesitamos conocer tu ubicación actual
          </p>
          <div className="loading-spinner mx-auto mb-4"></div>
          <p className="text-white opacity-75">Esperando selección de origen...</p>
        </div>
      </div>
    )}
  </div>
